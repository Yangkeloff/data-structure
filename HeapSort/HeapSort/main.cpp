...
// 堆排序
// 每一趟将堆顶元素加入有序子序列（与堆底元素，即待排序序列中的最后一个元素交换）
// 并将待排序元素序列再次调整为大根堆（小元素不断“下坠 ”）

//建立大根堆（大根堆：完全二叉树中，根结点值大于等于左右孩子）
void BuildMaxHeap(int A[],int len){
    for (int i=len/2; i>0; i--) {   // 从后向前遍历所有非终端结点，进行调整
        HeadAdjust(A,i,len);
    }
}
...
void HeadAdjust(int A[],int k,int len){     // 将以k为根的子树调整为大根堆
    A[0]=A[k];      // 暂存要调整结点的值
    for (int i=2*k; i<=len; i*=2) {     // 2k即当前结点k的左孩子，沿较大的子结点向下筛选
        if (i<len && A[i]<A[i+1])   // i=2k为左孩子,i+1即为右孩子,i<len则无右兄弟
            i++;    // 对比两者，若右孩子更大则i++
        
        if (A[0]>A[i]) break;   // 判断k结点是否大于其左右孩子，如果是，则跳出本轮循环
...
void HeapSort(int A[],int len){
    BuildMaxHeap(A, len);
    for (int i=len; i>0; i--) {   // i指向当前待排序元素序列的最后（堆底元素）
